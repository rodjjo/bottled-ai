...
def python_dir():
    result = os.path.join(root_dir(), 'bottled_ai', 'bin')
...
def build_diffusion_expert(args):
    ...
    subprocess.check_call([
        'cmake', '--install', '.', '--prefix', os.path.join(build_dir(), 'bottled_ai')
    ] , cwd=build_dir())
...
def publish_binaries():
    ...
    files = [
        os.path.join(source_dir, 'bottled-ai-win.exe'),
        os.path.join(source_dir, 'bottled-ai.exe'),
    ]
    for f in files:
